...
class Agent:
    ...
    def exists_action(state: AgentState):
        ...
        # Simple loop breaker: stop if too many tool calls made recently
        recent_tool_calls = [
            m for m in messages[-4:] if hasattr(m, 'tool_calls') and m.tool_calls
        ]
        if len(recent_tool_calls) >= 3:
            print("Too many tool calls, stopping.")
            return 'email_sender'  # or a fallback state
        result = messages[-1]
    ...
    # def call_tools_llm(self, state: AgentState):
    #     messages = state['messages']
    #     if not isinstance(messages[0], SystemMessage) or TOOLS_SYSTEM_PROMPT not in messages[0].content:
    #         messages = [SystemMessage(content=TOOLS_SYSTEM_PROMPT)] + messages
    #     message = self._tools_llm.invoke(messages)
    ...
    #         else:
    #             result = self._tools[t['name']].invoke(t['args'])
    #         # Use correct ToolMessage (or FunctionMessage if required by framework)
    ...
    #     # print(f"[DEBUG] Tool {t['name']} output: {result}")
